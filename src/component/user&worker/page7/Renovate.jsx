...
function Page7() {
  ...
  const [nextstps, setnextsteps] = useState(() => Number(localStorage.getItem('nextstpsR')) || 1);
  const [currentsteo, setcurrentsteo] = useState(() => Number(localStorage.getItem('currentsteoR')) || 0);
  ...
  const handelsteps=async(e)=>{
    ...
    try {
      ...
      localStorage.setItem('nextstpsR', updatedNextStps.toString());
      localStorage.setItem('currentsteoR', updatecureentstep.toString());
    } catch (error) {
      console.log(erro)
    }
  }
  ...
  const handleGoBack = () => {
    setnextsteps(1);
    setcurrentsteo(0);
    localStorage.setItem('nextstpsR', '1');
    localStorage.setItem('currentsteoR', '0');
  };
